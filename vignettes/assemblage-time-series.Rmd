...
We can plot grid values at different time slices or a summary metric considering all time slices
...
sf_mean_site <- 
  ...
  ungroup() |> 
  group_by(site) |> 
  mutate(mean.site = mean(mpd.obs, na.rm = TRUE), 
         var.site = var(mpd.obs, na.rm = TRUE)
         )

sf_grid_metric <- 
  df_assemblage_reg_mpd$grid |> 
  mutate(grid_id = as.character(grid_id)) |> 
  left_join(sf_mean_site, by = c(grid_id = "site")) 
...
continents <- rnaturalearth::ne_countries(scale = "medium", returnclass = "sf")
sf_mean_site2 <- sf::st_transform(sf_grid_metric, crs = "+proj=moll +x_0=0 +y_0=0 +lat_0=0 +lon_0=0")
continents <- sf::st_transform(continents, crs = "+proj=moll +x_0=0 +y_0=0 +lat_0=0 +lon_0=0")



ggplot() +
  geom_sf(data = sf_mean_site2, aes(fill = mean.site)) +  # Your data
  geom_sf(data = continents, fill = NA, color = "black") +  # Continent boundaries
  scale_fill_viridis_c(name = "plasma") +  # Optional: Use a color scale
  theme_minimal() +
  labs(title = "",
       fill = "Mean mpd by site")
