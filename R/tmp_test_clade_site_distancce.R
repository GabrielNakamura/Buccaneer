...
clade_site_distance <-
  function(df.TS.TE,
    ...
    if(!is.null(group) == TRUE){
      df.TS.TE <- df.TS.TE[, c(species, trait, TS, TE, group, lat, lng, site)]
      if(is.null(trait) == TRUE){
        colnames(df.TS.TE) <- c("species", "TS", "TE", "group", "lat", "lng", "site")
      } else{
        colnames(df.TS.TE) <- c("species", "trait", "TS", "TE", "group", "lat", "lng", "site")
      }
    } else{
      df.TS.TE <- df.TS.TE[, c(species, trait, TS, TE, lat, lng, site)]
      if(is.null(trait) == TRUE){
        colnames(df.TS.TE) <- c("species", "TS", "TE", "lat", "lgn", "site")
      } else{
        colnames(df.TS.TE) <- c("species", "trait", "TS", "TE", "lat", "lng", "site")
      }
    }
