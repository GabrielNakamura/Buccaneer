...
library(tidyr)
library(dplyr)
load(here::here("inst", "extdata", "df_can.Rdata")) # canidae occurrences
load(here::here("inst", "extdata", "longevities.Rdata")) # canidae longevities 
```

Computing mean longevities from all pyrate replicates
...
That is LD1, a proxy for the level of carnivory/herbivory and body mass
...
Coexisting metrics for grid scale

```{r}
res_clade_distance_assemblage100 <- 
...
res_clade_distance_assemblage50 <- 
  clade_site_distance(df.TS.TE = df_longevities2, 
                      df.occ = df_grid50, 
                      time.slice = 0.1, 
                      dist.trait = dist_traits, 
...
plot_assemblage_mnnd_raw_100 <- 
  ggplot(res_clade_distance_assemblage_nearest, aes(x = time.slice, y = mean.distance)) +
  geom_line(color = "grey50", alpha = 0.4) +  # Transparent line
...
ggsave(here::here("inst", "extdata", "figures_canidae", "plot_assemblage_mnnd_raw100.png"),
       plot_assemblage_mnnd_raw_100, 
       width = 10, 
       height = 10, 
       dpi = 800,
